...
import 'package:hc_management_app/shared/widgets/custom_date_picker/custom_date_picker_config.dart';
...
class CustomDatePicker extends StatefulWidget {
  CustomDatePicker({
    ...
    var isInitialValueValid = initialValue.length > 1
        ? (config.calendarType == CustomDatePickerType.range
            ? (initialValue[0] == null
...
class _CustomDatePickerState extends State<CustomDatePicker> {
  ...
  void _handleDayChanged(DateTime value) {
    ...
    setState(() {
      ...
      if (widget.config.calendarType == CustomDatePickerType.single) {
        selectedDates = [value];
      } else if (widget.config.calendarType == CustomDatePickerType.range) {
...
class _DayPickerState extends State<_DayPicker> {
  ...
  Widget build(BuildContext context) {
    ...
    final Color enabledDayColor = AppColors.black;
    final Color disabledDayColor = AppColors.grey600!;
    final Color selectedDayColor = AppColors.primary;
    final Color selectedDayBackground = AppColors.primary;
    final Color todayColor = AppColors.primary;
    ...
    while (day < daysInMonth) {
      ...
      if (day < 1) {
        ...
        List<Month> enableDays = widget.config.enableDays ?? [];

        bool dayIsDisabled = (enableDays.isNotEmpty)
            ? !(enableDays.contains(Month(id: month, day: day)))
            : false;
        ...
        if (widget.config.calendarType == CustomDatePickerType.range) {
...
/// [CustomDatePicker], or [showDatePicker2] which create full date pickers.
///
/// See also:
///
///  * [CustomDatePicker], which provides a Material Design date picker
///    interface.
///
...
setCustomDatePickerConfig(BuildContext context,
    {bool isDateRange = false,
    DateTime? firstDate,
    DateTime? lastDate,
    bool enableLastDate = false,
  ...
  return CustomDatePickerConfig(
    ...
    calendarType:
        isDateRange ? CustomDatePickerType.range : CustomDatePickerType.single,
    ...
    controlsHeight: SizeUtils.baseWidthHeight48,
